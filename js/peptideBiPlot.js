$(document).ready(function(){
  $.ajax({
    ...
    geneSymbolKeggMap = geneSymbolKeggMap.split('\n').filter(function(d){return d.split('\t')[3] != 'uncharacterized';})
      .reduce(function(result,item){var a = item.split('\t'); result[a[3]]=a[1]; return result;},{});
    $.ajax({
      url:'data/pathwayList.txt'
    }).done(function(pathwayList){
      pathwayList = pathwayList.split('\n').reduce(function(accumulator,d){var a = d.split('\t');accumulator[a[0].replace('path:map','hsa')] = a[1];return accumulator;},{});
    $.ajax({
      ...
      var yValueKey = 'P-IC50-Correlation',
        ...
        var createPanel = function(dom,data){
          ...
          if(sameProtein.length >0){
            ...
            .text(function(p){return p.Peptide+ ' ('+getName(xValueKey)[0]+':'+parseFloat(p[xValueKey]).toFixed(3)+', '+getName(yValueKey)[0]+':'+parseFloat(p[yValueKey]).toFixed(3)+')';})
          ...
          if(geneSymbolKeggMap.hasOwnProperty(proteinName) && pathwayMap.hasOwnProperty(parseInt(geneSymbolKeggMap[proteinName].replace('hsa:','')))){
            d3.select('#pathwayProtein_'+peptideName).selectAll('div')
